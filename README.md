...
Откройте **SQL Editor** и выполните [supabase/migrations/202609220001_blog.sql](supabase/migrations/202609220001_blog.sql) целиком. Скрипт создаёт таблицы, индексы, триггеры дат, RLS, политики Storage и атомарную функцию просмотров. Проверьте, что появились `categories`, `articles`, `media` и bucket `article-images`.
