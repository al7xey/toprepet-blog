...
- Защищённая админка: рубрики, список и редактор статей, публикация, предпросмотр, медиа.
...
Откройте **SQL Editor** и по порядку выполните все файлы из `supabase/migrations`: сначала [202609220001_blog.sql](supabase/migrations/202609220001_blog.sql), затем [202609230002_hardening.sql](supabase/migrations/202609230002_hardening.sql) и [202609230003_refactor.sql](supabase/migrations/202609230003_refactor.sql). Миграции создают таблицы, индексы, триггеры дат, защиту иерархии от циклов, RLS, политики Storage и атомарную функцию просмотров. Проверьте, что появились `categories`, `articles`, `media`, `article_previews`, `article_slug_redirects` и bucket `article-images`.
...
Откройте `http://localhost:3000/admin/login`, войдите и проверьте список статей по адресу `/admin/articles`. Создайте рубрику в `/admin/categories`, затем откройте `/admin/articles/new`: укажите заголовок и рубрику, напишите текст и нажмите «Опубликовать». При необходимости сохраните черновик или откройте предпросмотр. Готовая статья появится по адресу `/articles/ваш-slug`.

## 5. Как работать со статьями

Заголовок H1 задаётся вне редактора. Адрес статьи предлагается из русского заголовка, а краткое описание — из начала текста; оба можно изменить в «Адрес и настройки». В Tiptap доступны абзацы, H2/H3, выделение, списки, цитаты, ссылки, разделитель, история и изображения. Произвольные шрифты, размеры, цвета и HTML редактор не принимает.

У каждого изображения обязателен alt, подпись необязательна. При первой загрузке изображения черновик создаётся автоматически. Перед upload браузер преобразует JPG/PNG/WebP в WebP, ограничивает размер и не сохраняет исходный файл. Оптимизированный файл отправляется в `article-images`; сервер снова проверяет MIME, реальные размеры и вес. После удаления изображения из редактора сохраните статью: только после успешного сохранения удаляются запись `media` и объект Storage.

Предпросмотр показывает текущие несохранённые изменения в отдельной вкладке и не меняет опубликованную статью. Статья становится видимой только после публикации. При первой публикации БД ставит `published_at`, а `modified_at` остаётся пустым. При редактировании уже опубликованной статьи `published_at` сохраняется и устанавливается `modified_at`. Снятие с публикации и повторная публикация сохраняют исходную дату `published_at`; редакционные изменения обновляют только `modified_at`, а просмотры эти даты не меняют.
