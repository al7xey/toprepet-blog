...
export function CategoriesManager({ categories }: { categories: Category[] }) {
  ...
  return <div className="mt-7 grid max-w-[950px] gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
    <form onSubmit={save} className="paper h-fit space-y-5 p-5 sm:p-7">
      ...
      <div><label htmlFor="category-name" className="label">Название</label><input id="category-name" className="field" required value={name} placeholder="Например, Информатика" onChange={event => { setName(event.target.value); if (!slugTouched) setSlug(makeSlug(event.target.value)) }}/></div>
      <div><label htmlFor="category-parent" className="label">Куда добавить</label><select id="category-parent" className="field" value={parent} onChange={event => setParent(event.target.value)}><option value="">Новый раздел</option>{categories.filter(category => !excludedParents.has(category.id)).map(category => <option key={category.id} value={category.id}>Внутрь: {label(category)}</option>)}</select><p className="mt-2 text-xs text-[#697383]">Так собирается путь: ЕГЭ → Информатика → Задание 12.</p></div>
      <details className="border-t border-[#eee5de] pt-4"><summary className="cursor-pointer text-sm font-semibold">Описание и адрес</summary><div className="mt-4 space-y-4"><div><label htmlFor="category-description" className="label">Описание</label><textarea id="category-description" className="field" rows={3} value={description} onChange={event => setDescription(event.target.value)}/></div><div><label htmlFor="category-slug" className="label">Адрес</label><input id="category-slug" className="field" required pattern="[a-z0-9]+(-[a-z0-9]+)*" value={slug} onChange={event => { setSlugTouched(true); setSlug(event.target.value) }}/></div><div><label htmlFor="category-order" className="label">Порядок</label><input id="category-order" type="number" className="field" value={order} onChange={event => setOrder(Number(event.target.value))}/></div></div></details>
