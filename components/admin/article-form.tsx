...
export function ArticleForm({ article, categories }: { article?: Article; categories: Category[] }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('beforeunload', onBeforeUnload); document.addEventListener('click', onClick, true)
  ...
  return <main><div className="mb-8 flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-[#df5b27]">{id ? 'Редактирование' : 'Новая статья'}</p><h1 className="mt-2 text-4xl font-extrabold">{title || 'Без названия'}</h1><span className="mt-2 block text-sm text-[#687482]">{status === 'published' ? 'Опубликована' : 'Черновик'}{dirty ? ' · Несохранённые изменения' : ''}</span></div><div className="flex flex-wrap gap-2"><button type="button" disabled={busy} className="button-outline" onClick={()=>perform('draft')}>Сохранить черновик</button><button type="button" disabled={busy} className="button-outline" onClick={preview}>Предпросмотр</button>{status === 'published' && <button type="button" disabled={busy} className="button-outline" onClick={()=>perform('draft')}>Снять с публикации</button>}<button type="button" disabled={busy} className="button-primary" onClick={()=>perform('published')}>Опубликовать</button></div></div>
