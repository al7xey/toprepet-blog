...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!name || !/^[a-z0-9]+(-[a-z0-9]+)*$/.test(slug) || body.parent_id === id) return errorJson('Проверьте название, адрес и родительскую рубрику')
  const db = auth.session!.supabase
  if (body.parent_id) {
    const { data: all } = await db.from('categories').select('id,parent_id')
    let parent = body.parent_id
    const seen = new Set([id])
    while (parent) { if (seen.has(parent)) return errorJson('Рубрика не может быть собственным потомком'); seen.add(parent); parent = all?.find(c => c.id === parent)?.parent_id || null }
  }
