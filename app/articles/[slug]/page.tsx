...
import { articleSlugRedirect, categories, publishedArticle, relatedArticleCards } from '@/lib/data-server'
...
export default async function ArticlePage({ params }: Props) {
  ...
  const primary = await relatedArticleCards(article.id, [article.category_id], 3)
  ...
  const fallback = primary.length < 3 ? await relatedArticleCards(article.id, fallbackIds, 3 - primary.length) : []
  const url = siteUrl(`/articles/${article.slug}`)
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schemas).replace(/</g, '\\u003c') }}/><ArticleView article={article} categories={all} related={[...primary, ...fallback]}/></>
}
