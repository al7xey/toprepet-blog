...
export default async function ArticlePage({ params }: Props) {
  ...
  return <main className="page-shell py-12 sm:py-16"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schemas).replace(/</g, '\\u003c') }}/><Breadcrumbs category={category} categories={all} articleTitle={article.title}/><article><header className="mx-auto max-w-[800px]">{category && <a href={rubricHref(category, all)} className="eyebrow hover:underline">{category.name}</a>}<h1 className="mt-5 text-4xl font-extrabold leading-[1.15] tracking-[-.055em] sm:text-6xl">{article.title}</h1><p className="mt-6 text-lg leading-8 text-[#697383] sm:text-xl">{article.excerpt}</p><div className="mt-9 flex flex-wrap items-center justify-between gap-5 border-y border-[#eee5de] py-5"><div className="text-sm leading-6 text-[#697383]"><strong className="block text-[#202838]">{article.author_name}</strong><time dateTime={article.published_at || undefined}>Опубликовано {displayDate(article.published_at)}</time>{article.modified_at && <><span> · </span><time dateTime={article.modified_at}>Обновлено {displayDate(article.modified_at)}</time></>}<span className="block">{displayViews(article.view_count)} · {displayReadingTime(article.content_json)}</span></div><ArticleInteractions id={article.id} title={article.title} url={url}/></div></header>
    ...
    <div className="mx-auto grid max-w-[1080px] gap-12 lg:grid-cols-[minmax(0,800px)_220px]"><div className="min-w-0"><ArticleToc headings={headings} variant="mobile"/><section aria-label="Текст статьи" className="article-body" dangerouslySetInnerHTML={{ __html: html }}/></div><ArticleToc headings={headings} variant="desktop"/></div></article>
